...
function canReach(arr: number[], start: number): boolean {
    const q: Step[] = [{ pos: start, visited: [] }];
    let s: Step | undefined;

    while ((s = q.shift()) !== undefined) {
        //console.log({pos})
        if (arr[s.pos] === 0) return true;
        // s.visited = [...s.visited];
        s.visited[s.pos] = true;

        const forward = s.pos + arr[s.pos]
        const backward = s.pos - arr[s.pos]

        if (forward < arr.length && !s.visited[forward]) {
            //console.log({forward})
            q.push({ pos: forward, visited: s.visited })
        }
        if (0 <= backward && !s.visited[backward]) {
            //console.log({backward})
            q.push({ pos: backward, visited: s.visited })
        }
    }
    return false;
};
...
Deno.test('1', () => {
    ...
    assertEquals(canReach(arr, start), false)
})
