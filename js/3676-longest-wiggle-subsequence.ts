...
/* dp[i] = longest wiggle sequence for dp[0..i]

dp[0] = 1
dp[i] = if nums[i] can extends 
*/

const wiggleMaxLength = wiggleMaxLengthDP;

function wiggleMaxLengthLinear(nums: number[]): number {
    if (nums.length == 1) return 1;
    let cnt = (nums[0] == nums[1]) ? 1 : 2;
    if (nums.length == 2) return cnt;
    let prevDiff = nums[1] - nums[0];
    for (let i = 2; i < nums.length; i++) {
        let curDiff = nums[i] - nums[i - 1];
        if (curDiff == 0) continue;
        if (prevDiff == 0 || curDiff * prevDiff < 0) cnt++;
        prevDiff = curDiff;
    }
    return cnt;
}
...
function wiggleMaxLengthDP(nums: number[]): number {
    const up = Array<number>(nums.length);
    const down = Array<number>(nums.length);
    up[0] = 1;
    down[0] = 1;
    let max = 0;
...
nums = [1, 7, 4, 9, 2, 5];
assertEquals(wiggleMaxLength(nums), 6);

nums = [1, 17, 5, 10, 13, 15, 10, 5, 16, 8];
assertEquals(wiggleMaxLength(nums), 7);

nums = [1, 2, 3, 4, 5, 6, 7, 8, 9];
assertEquals(wiggleMaxLength(nums), 2);

nums = [0, 1];
assertEquals(wiggleMaxLength(nums), 2);

nums = [0, 0];
assertEquals(wiggleMaxLength(nums), 1);

nums = [0, 0, 0];
assertEquals(wiggleMaxLength(nums), 1);

nums = [3, 3, 3, 2, 5];
assertEquals(wiggleMaxLength(nums), 3);
...
assertEquals(wiggleMaxLength(nums), 334);
